...
sim_data <- function(nstudies, ncoef,nchoose = 10, ntest,hetero,inbalance,simsep){
 ...
 # totalsize = sum(cluSize1)
  ...
  numtests = rep(0,nstudies)
  
  aa = sample(1:nstudies,1)
  numtests[aa] = 1
  ...
  clus_size = runif(nstudies,0.95,1.05)
    
  clus_size = floor(clus_size*totalsize/nstudies)
...
vary_levels <- function(reps, var_list, modfit, modpred, ndat, ntest,ncoef,nchoose,out_str,k,hetero,inbalance,simsep){
  ...
  for (i in 1:length(var_list)){
    ...
    level_rep <- rep.clusters_fit(reps, modfit, modpred, ndat= level, ntest, ncoef = ncoef, nchoose = nchoose,k = k, hetero = hetero,inbalance=inbalance,simsep = simsep)
    #means

...
sim_data <- function(nstudies, ncoef,nchoose = 10, ntest,hetero,inbalance,simsep, testalloc){
  ...
  #quadatic heterogeniety while balancing signal strength
  icoefs2 <- sample(c(runif(2, -4, -1), runif(2, 1, 4)))
  #icoefs2 <- icoefs2[1:2]
  vars3 <- sample(1:ncoef, 4)
  ...
  for (i in 1:nstudies){
    ...
    #quadratic heterogeniety
    y2 <- gamma[i]*hetero*(icoefs2[1]*(data_i[,vars3[1]])^2 + icoefs2[2]*(data_i[,vars3[2]])^2 )
  #                           +icoefs2[3]*(data_i[,vars3[3]])^2+icoefs2[4]*(data_i[,vars3[4]])^2)
    
    y3 <- gamma[i]*hetero*(as.matrix((data_i[,vars2])) %*% coefs2)
...
rep.clusters_fit <- function(reps, modfit, modpred, ndat, ntest, ncoef,nchoose, k,hetero,inbalance,simsep){
  ...
  results <- foreach(i = 1:reps, .combine = 'rcomb', .multicombine = TRUE, .export = ls(globalenv())) %dopar% {
    ...
    sd <- sim_data(ndat, ncoef = ncoef,nchoose = nchoose, ntest = ntest,hetero = hetero,inbalance = inbalance,simsep = simsep,testalloc= testalloc)$studies_list
...
simsep = 0.7
cs <- vary_levels(reps = 30, var_list = k_list, modfit = randomforestfit, modpred

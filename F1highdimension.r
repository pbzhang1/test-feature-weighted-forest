...
TestAllocation <- function(nstudies,iters = 1000000){
  M = matrix(0,nrow = iters,ncol = nstudies)
  N = rep(0,iters)
  k = sqrt(nstudies)
  for (i in 1:iters){
    x = runif(nstudies)
...
findTestAllocation <- function(N, inbalance){
  mm = 1
  y = 1
  ...
  for (i in 1:length(N)){
    ...
    if (abs(N[i]-inbalance)<mm){
      y = i
      mm = abs(N[i]-inbalance)
    }
  }
  return (y)
...
sim_data <- function(nstudies, ncoef,nchoose = 10, ntest,hetero,inbalance,simsep, testalloc,signalstrength =10){
  ...
  coefs = coefs * signalstrength/norm_vec(coefs)
  ...
  coefs2 <- coefs2*sqrt(mean(abs(coefs^2)))/sqrt(mean(abs(coefs2^2)))
  #vars = vars+1
  icoefs <- c(3.5, 0.7)*signalstrength/10
  ...
  inbalance2 = inbalance*n2
  ind = findTestAllocation(testalloc$N,inbalance2)
  
  
  numtests = testalloc$M[ind,]
  ...
  #each cluster minimum 400 points
...
checksilhouette <- function(simsep){
...
#merge the data 
#permutation the data, then use k-means 
...
#####
# create_random <- function(studies_list, ntest, nsep){
...
clusters_fit <- function(modfit, modpred, ndat, ncoef, ntest, studies_list, cluster_ind, nsep){
  ...
  if (cluster_ind == 1){ #K-means clustering
    ...
    edat <- create_random(studies_list, ntest, nsep)
    len_clust <- sapply(edat, function(i) nrow(i))
...
rep.clusters_fit <- function(reps, modfit, modpred, ndat, ntest, ncoef,nchoose, k,hetero, inbalance, simsep, signalstrength){
  #####
  ...
  testalloc = TestAllocation(ndat,iters = 1000000)
  ...
  results <- foreach(i = 1:reps, .combine = 'rcomb', .multicombine = TRUE, .export = ls(globalenv())) %dopar% {
    ...
    sd <- sim_data(ndat, ncoef = ncoef,nchoose = nchoose, ntest = ntest,hetero = hetero,inbalance = inbalance,simsep = simsep,testalloc= testalloc,signalstrength=signalstrength)$studies_list
...
vary_levels <- function(reps, var_list, modfit, modpred, ndat, ntest,ncoef,nchoose,out_str,k,hetero,inbalance,simsep){
  ...
  for (i in 1:length(var_list)){
    ...
    level_rep <- rep.clusters_fit(reps, modfit, modpred, ndat, ntest, ncoef = ncoef, nchoose = nchoose,k = k, hetero = hetero,inbalance=inbalance,simsep = level,signalstrength = 5 )
    #means

...
#simsep how much the cluster are separated at generation step
...
#cluster_ind = 1 if running the algorithm on the k-means clusters, 4 for test feature weighted forest
...
vary_levels <- function(reps, var_list, modfit, modpred, ndat, ntest,ncoef,nchoose,out_str,k,hetero,inbalance,simsep){
  ...
  for (i in 1:length(var_list)){
    ...
    level_rep <- rep.clusters_fit(reps, modfit, modpred, ndat, ntest, ncoef = ncoef, nchoose = nchoose,k = k, hetero = hetero,inbalance=inbalance,simsep = simsep,signalstrength =level )
    #means
